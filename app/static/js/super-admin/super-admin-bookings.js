...
var editBookingCustomForm = document.getElementById("edit-booking-custom");
...
var editBookingDuration = document.getElementById("edit-duration");
...
fetch("/superAdminPannel/getAllBookings").then((response) => {
    if (response.ok) {
...
function setEditDuration() {
    ...
    if (isNaN(days) || isNaN(hours) || isNaN(mins)) {
        editBookingDuration.innerText = `0j 0h0mins`
    } else {
        editBookingDuration.innerText = `${days}j ${hours}h${mins}mins`
    }
}
...
function deleteBooking(url, usrId, bookingId, displayDialog) {
...
async function postEditBooking(url, data) {
...
async function getEditBookingDataAndPost(booking) {
    ...
    if (ddS.getTime() === ddE.getTime() || ddE < ddS) {
        ...
        return await postEditBooking(editBookingForm.action, data);
    }
}
...
function openEditBookingDialog(bookingId) {
    ...
    if (typeof editBookingDialog.showModal === "function") {
        fetch('/superAdminPannel/fetchUserPlates?' + new URLSearchParams({
...
editBookingPeriodSelector.addEventListener("input", (e) => {
...
editBookingForm.addEventListener('submit', (e) => {
    ...
    (async () => {
        ...
        if (result) {
            deleteResponse = await deleteBooking("/superAdminPannel/deleteUserBooking", currentEditingBooking['userId'], currentEditingBooking['bookingID']);
...
askDeleteDialog.querySelector("form").addEventListener("submit", (e) => {
    e.preventDefault();
    deleteBooking(e.target.action, currentEditingBooking['userId'], currentEditingBooking['bookingID'], true);
});