...
function fetchUserPlates(usrId) {
    fetch('/superAdminPannel/fetchUserPlates?' + new URLSearchParams({ 'usrID': usrId }), {
        method: "GET"
    }).then((response) => {
        if (response.ok) {
...
function feedUserAndPlatesSelector() {
    fetch("/superAdminPannel/getAllUsers", {
        method: "GET"
    }).then((response) => {
        if (response.ok) {
            return response.json();
        ...
        clearAddSelector(addBookingUserSelector);
        for (let u of data) {
...
function clearAddSelector(select) {
...
addBookingForm.addEventListener('submit', (e) => {
    ...
    result = await getBookingDataAndPost();
        ...
        if (!result) {
            ...
            location.href = "/superAdminPannel/bookings";
        }
    })();
});
